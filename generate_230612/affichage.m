...
dir = ['Dataprocess/AR/Healthy/']; dir = dir(ones(ncows,1),:);
mat = '.mat'; mat = mat(ones(ncows,1),:);
filenames = [dir num2str(cows') mat];
...
for icow=1:ncows
    spent_time(icow,:)=data_real{icow}.model_cow.spent_time;
    spent_time_sim(icow,:)=data_sim{icow}.sim_cow.spent_time_sim;
end
%% Plot
b = plotaveragetime(cows,spent_time,spent_time_sim)
%% Distribution of individual cow act per hour
Mf = data_real{1}.model_cow.Mf;
std_dist = data_real{1}.model_cow.std_dist;
Ms = data_sim{1}.sim_cow.Ms;
std_dist_sim = data_sim{1}.sim_cow.std_dist_sim;
...
for i =1:ncows
    all_Mf(:,:,i) = data_real{i}.model_cow.Mf;
    all_std_dist(:,:,i) = data_real{i}.model_cow.std_dist;
    all_Ms(:,:,i) = data_sim{i}.sim_cow.Ms;
    all_std_dist_sim(:,:,i) = data_sim{i}.sim_cow.std_dist_sim;
end
...
for i=1:ncows
    all_AR_h{i} = data_ar_healthy{i}.ar_cow_healthy.AR_healthy;
end
mean2 = @(x)(mean(x,2));
...
for i=1:ncows
    all_AR_h_sim{i} = data_sim{i}.sim_cow.AR;
end
mean2 = @(x)(mean(x,2));
...
icowlist = 1:3;
for icow = icowlist
    ARcow = data_ar_healthy{icow}.ar_cow_healthy.AR_healthy;
...
%%
for i=1:8
    AR_uh = data_ar_uh{i}.ar_uh.AR_uh;
    plot(t, mean(AR_uh,2), 'LineWidth', 1,'LineWidth',2,"DisplayName",data_ar_uh{i}.ar_uh.state);
end
legend()
set(gcf,'Color',[1 1 1])
ax = gca;
